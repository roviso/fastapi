...
def show_user_name(id,le_user_mapping):
    print(le_user_mapping[25])
...
def get_urls(user,url_list, model,user_le_mapping, url_to_keyword_label):
    print('url_list used: ',url_list,len(url_list))
    top_20 = test_utils.get_top_20(user,url_list, model,user_le_mapping, url_to_keyword_label)
    print('top_20: ',top_20)
    if not top_20:
        raise HTTPException(status_code =  status.HTTP_404_NOT_FOUND,
                            detail = f"NO recommendation found")
    return {'Top recommendation':top_20}
